...
````yaml /specifications/deribit_openapi.json get /private/change_scope_in_api_key
openapi: 3.0.0
info:
  title: Deribit API
  version: 2.1.1
  ...
  /private/change_scope_in_api_key:
    get:
      tags:
        ...
        - Private
      description: >+
        Updates the scope (permissions) for an existing API key. The scope
        determines which operations the API key can perform (e.g., read-only,
        read-write, trading, etc.).


        When the scope is changed, the API key immediately loses access to
        operations outside the new scope. Applications using this key may need
        ...
        **Scope:** `account:read_write`
        ...
        [Try in API
        console](https://test.deribit.com/api_console?method=%2Fprivate%2Fchange_scope_in_api_key)
