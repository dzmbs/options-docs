...
````yaml /specifications/deribit_openapi.json get /private/get_order_history_by_instrument
openapi: 3.0.0
info:
  title: Deribit API
  version: 2.1.1
  ...
  /private/get_order_history_by_instrument:
    get:
      tags:
        ...
        - Private
      description: >+
        Retrieves the order history for a specific instrument. The history
        ...
        Results can be paginated using `offset` and `count` parameters, or using
        continuation tokens. Use `include_old_orders` to include orders from
        before a certain date, and `historical` to retrieve historical order
        data.
        ...
        **Scope:** `trade:read`
        ...
        [Try in API
        console](https://test.deribit.com/api_console?method=%2Fprivate%2Fget_order_history_by_instrument)
