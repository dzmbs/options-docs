...
````yaml /specifications/deribit_openapi.json get /private/get_order_state
openapi: 3.0.0
info:
  title: Deribit API
  version: 2.1.1
  ...
  /private/get_order_state:
    get:
      tags:
        ...
        - Private
      description: >+
        Retrieves the current state of an order identified by its order ID. The
        response includes order details such as status (open, filled,
        cancelled), filled amount, remaining amount, price, and other order
        properties.


        **Important Note for Mass Quotes:** Quote orders are order-like
        ...
        **Scope:** `trade:read`
        ...
        [Try in API
        console](https://test.deribit.com/api_console?method=%2Fprivate%2Fget_order_state)
